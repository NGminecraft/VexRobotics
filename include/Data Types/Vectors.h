...
public:
	...
	template <Numeric Type, size_t Len, bool O>
	VectorBase<T, N, Owner>& operator*=(VectorBase<Type, Len, O> vec) requires (Len == N) {
		for (size_t i = 0; i < N; i++) {
			operator[](i) *= vec[i];
		}
		return *this;
	...
	template <Numeric Type, size_t Len, bool O>
	VectorBase<T, N, true> operator*(VectorBase<Type, Len, O> vec) requires (Len == N) {
